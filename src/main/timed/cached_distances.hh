#ifndef CACHED_DISTANCES_HH
#define CACHED_DISTANCES_HH
...
class CachedDistances : public TimedDistanceFunc
{
private:
...
public:
  CachedDistances(TimedRouter& router,
                  TimedEdgeFunc<num>& costs,
                  idx capacity = 100000)
    : router(router),
      costs(costs),
      cache(std::bind(&CachedDistances::shortestPath, this, std::placeholders::_1), capacity)
  {}

  num operator()(const Vertex& vertex, const Vertex& target, idx departureTime) override;
};

#endif /* CACHED_DISTANCES_HH */
